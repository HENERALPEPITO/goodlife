  ...
  const fetchBalance = useCallback(async () => {
    try {
      ...
      if (json.error) {
        ...
        setAdvancePayment(json.advancePayment || 0);
      }
    } catch (error) {
              ...
              <div className="flex items-center gap-4 flex-shrink-0">
                ...
                {advancePayment > 0 && (
                  <div className="text-right border-l border-gray-300 pl-4">
                    <p className="text-xs font-medium text-gray-500 uppercase tracking-wider">Advance Payment</p>
                    <p className="text-2xl font-bold text-amber-600 mt-0.5 tabular-nums">${advancePayment.toFixed(2)}</p>
                  </div>
                )}
                <Button
